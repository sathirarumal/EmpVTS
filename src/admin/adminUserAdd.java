...
/**
 * Servlet implementation class adminDriverAdd
 */
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		Connection con = DBConnection.createConnection();
		 ...
		 try { String query ="insert into user_profile (emp_No,pw,vehi_No,rout_No,status,Type) values (?,?,?,?,?,?)";
		  ...
		  String message = "Success"; response.sendRedirect("admin_User.jsp?message="+ URLEncoder.encode(message, "UTF-8"));
		  
		  } catch(SQLException e) { 
			  
			  String message = "Some thing wrong"; response.sendRedirect("admin_User.jsp?message="+ URLEncoder.encode(message, "UTF-8"));
			  e.printStackTrace(); 		  
		  }
	}
